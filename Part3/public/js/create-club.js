...
// Require login, then load current user + (if editing) the club to pre-fill.
(async function initCreateClub() {
  const username = Api.currentUsername();
  if (!username) { window.location.href = "login.html"; return; }
  try { currentUserData = await Api.getUser(username); } catch (e) { currentUserData = null; }

  if (!isEditMode) return;

  try { editClub = await Api.getClub(editTitle); } catch (e) { editClub = null; }
...
// ============================================
document.getElementById("club-name").addEventListener("blur", async function () {
  if (isEditMode) return;
  const val = this.value.trim();
  if (!val) return;
  // GET /api/clubs/:title -> 200 means the name is taken, 404 means available.
  let exists = false;
  try { await Api.getClub(val); exists = true; } catch (e) { exists = false; }
  if (exists) {
...
document.getElementById("create-club-form").addEventListener("submit", async function (e) {
  ...
  // Guard: only one managed club allowed (skip in edit mode). Server enforces too.
  if (!isEditMode && currentUserData && currentUserData.managedClub) {
    const ge = document.getElementById("general-error");
    ge.textContent = `You already manage "${currentUserData.managedClub}". Each member can only manage one club.`;
  ...
  try {
    if (isEditMode) {
      await Api.updateClub(editTitle, {
        manager: username,
    ...
    const ge = document.getElementById("general-error");
    // Surface a duplicate-name conflict on the name field; otherwise show general error.
    if (!isEditMode && /name|exists/i.test(msg)) {
      showError("club-name-error", msg);
      setInvalid("club-name", true);
    }
    ge.textContent = msg;
    ge.style.display = "block";
